...
test('does not allow adding a task exceeding maximum length', () => {
    // Render the component
    render(<App />);
    ...
    // Enter a task exceeding the maximum length
    const inputElement = screen.getByPlaceholderText(/Add a new task/i);
    fireEvent.change(inputElement, { target: { value: 'Task'.repeat(100) } });
  
    // Click the "Add Task" button
    fireEvent.click(addButton);
  
    // Check if no task is added to the list
    const taskElements = screen.queryAllByText(/Test Task/i);
